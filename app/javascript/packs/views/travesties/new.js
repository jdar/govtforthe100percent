...
class NewTravestyForm {
  constructor(form) {
    ...
    this._form_container = $('.new-travesties-form-container');
  ...
  _bindPreviewButton() {
    this._previewButton.click(event => {
      ...
      const street = form.elements.travesty_street.value;
      const city = form.elements.travesty_city.value;
      const state = form.elements.travesty_state.value;
      const country = form.elements.travesty_country.value;
  ...
  _updateForm(coords, data, textStatus) {
    ...
    this._rebind();
    this._requestNearbyTravesties(coords);
    this._updateMap(coords);
  }

  _requestNearbyTravesties(coords) {
    $.ajax({
      type: 'GET',
      url: '/travesties',
      data: {
...
$(() => {
  $.fn.initializeNewTravestyForm = function() { return new NewTravestyForm($(this)); };
  $('.submit-new-bathroom-form').initializeNewTravestyForm();
});
