...
class NewExperienceForm {
  constructor(form) {
    ...
    this._form_container = $('.new-experiences-form-container');
  ...
  _bindPreviewButton() {
    this._previewButton.click(event => {
      ...
      const street = form.elements.experience_street.value;
      const city = form.elements.experience_city.value;
      const state = form.elements.experience_state.value;
      const country = form.elements.experience_country.value;
  ...
  _updateForm(coords, data, textStatus) {
    ...
    this._rebind();
    this._requestNearbyExperiences(coords);
    this._updateMap(coords);
  }

  _requestNearbyExperiences(coords) {
    $.ajax({
      type: 'GET',
      url: '/experiences',
      data: {
...
$(() => {
  $.fn.initializeNewExperienceForm = function() { return new NewExperienceForm($(this)); };
  $('.submit-new-bathroom-form').initializeNewExperienceForm();
});
