...
describe Travesty do
  let(:travesty) { described_class.new }

  describe '#rated?' do
    let(:unrated_travesty) { described_class.new }
    let(:rated_travesty)   { described_class.new(upvote: 1) }

    specify { expect(rated_travesty).to be_rated }
    specify { expect(unrated_travesty).not_to be_rated }
  end

  describe '#rating_percentage' do
    it 'returns 0 when there are no votes' do
      expect(travesty.rating_percentage).to eq 0
    end
  ...
  describe '.current' do
    it 'returns active listings' do
      create(:travesty, directions: "Most Recent")
      edit = create(:travesty)
      edit.update(approved: false, edit_id: 2)

      travesties = described_class.current

      expect(travesties.size).to eq(1)
      expect(travesties.first.directions).to eq("Most Recent")
    end

    it 'returns most recent edit approved' do
      travesty = create(:travesty, id: 1, edit_id: 1)
      edit1 = create(:travesty, id: 2, approved: true, directions: "Most Recent")
      edit1.update(edit_id: travesty.id)
      edit2 = create(:travesty, id: 3, directions: "Not approved", approved: false)
      edit2.update(edit_id: travesty.id)
